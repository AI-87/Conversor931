...
def procesar_931(file):
    with pdfplumber.open(file) as pdf:
        ...
        # -----------------------------
        for r in [1, 4, 8, 9, 10]:
            m = re.search(rf"Rem\. {r}[:\s]+([\d.,]+)", txt)
            res[f"Rem {r}"] = limpiar_monto(m.group(1)) if m else 0.0

        # -----------------------------
        # Detracciones
        # -----------------------------
        res["Ley 27430 Detraido"] = extraer_con_codigo(
            txt, "Detraido", r"Detraido[:\s]+([\d.,]+)"
        )
        res["Dec 394"] = extraer_con_codigo(
            txt, "394", r"394[:\s]+([\d.,]+)"
        )
        ...
        # -----------------------------
        res["351-Contrib SS Total"] = extraer_con_codigo(
            txt,
            "351-Contribuciones de Seguridad Social",
            r"Social\s+([\d.,]+)\s+a1",
        )
        res["351-SIPA"] = extraer_con_codigo(
            txt, "SIPA", r"SIPA\s+([\d.,]+)"
        )
        res["351-No SIPA"] = extraer_con_codigo(
            txt, "No SIPA", r"No SIPA\s+([\d.,]+)"
        )
        res["301-Aportes SS"] = extraer_con_codigo(
            txt,
            "301",
            r"301-?Aportes.*?Seguridad Social\s+([\d.,]+)",
        )

        res["352-Contrib OS"] = extraer_con_codigo(
            txt,
            "352",
            r"352-?Contrib.*?Obra Social\s+([\d.,]+)",
        )
        res["302-Aportes OS"] = extraer_con_codigo(
            txt,
            "302",
            r"302-?Aportes.*?Obra Social\s+([\d.,]+)",
        )

        res["312-LRT"] = extraer_con_codigo(
            txt, "312", r"312-?L\.?R\.?T\.?\s+([\d.,]+)"
        )
        res["028-Vida"] = extraer_con_codigo(
            txt, "028", r"028-?Seguro.*?Vida\s+([\d.,]+)"
        )
...
files = st.file_uploader(
    "Cargá tus F.931 (PDF)", type="pdf", accept_multiple_files=True
)
...
if files:
    try:
        ...
        empresa_elegida = st.selectbox(
            "Seleccioná la empresa para ver la planilla:",
            empresas_disponibles,
        )

        # Filtrar y trasponer
        df_final = (
            df[df["Empresa"] == empresa_elegida]
            .set_index("Mes - Año")
            .sort_index()
            .T
        )
        ...
        # -----------------------------
        # Formato seguro SOLO para números
        # -----------------------------
        filas_texto = ["Empresa", "CUIT", "Empleados"]

        styler = df_final.style.format(
            "{:,.2f}",
            subset=pd.IndexSlice[
                df_final.index.difference(filas_texto), :
            ],
        ...
        st.download_button(
            ...
            buffer.getvalue(),
            f"Planilla_{empresa_elegida}.xlsx",
        )

...
def limpiar_monto(texto):
    ...
    try:
        # Extrae solo dígitos, comas y puntos. Maneja negativos por si hay retenciones.
...
def procesar_931(file):
    with pdfplumber.open(file) as pdf:
        # Unimos el texto de todas las páginas para no perder datos en el medio
        txt = "\n".join([p.extract_text() for p in pdf.pages if p.extract_text()])
        res = {}
        
        # Identificación (Busca cualquier nombre después de 'Social:')
        ...
        # --- EXTRACCIÓN POR CÓDIGOS (Resuelve los 0.0) ---
        # Usamos búsquedas que ignoran el texto intermedio
        patrones = {
            '351-Contrib SS Total': r"351-?Contribuciones.*?Seguridad Social\s+([\d.,]+)",
...
if files:
    try:
        ...
        if not df.empty:
            ...
            emp_sel = st.selectbox("Seleccioná la empresa para generar la planilla:", empresas)
            
            # Filtramos por empresa y trasponemos
            df_final = df[df['Empresa'] == emp_sel].set_index('Mes - Año').T
            
            st.write(f"### Planilla Consolidada: {emp_sel}")
            # Se muestra la tabla nativa (sin estilo .format para evitar errores de tipo 'str')
